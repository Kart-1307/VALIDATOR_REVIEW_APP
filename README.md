...
The backend is powered by 6 primary tables with strict **Row-Level Security (RLS)** policies:
...
| `questions` | The main repository of SAT questions, checklists, overrides, and claims. | Select for all auth users. Insert/Delete for Admins. Update for all active users **except Auditors** (read-only is enforced at the database level, not just in the UI). |
| `audit_log` | Immutable append-only audit trail logging all reviewer operations. | Read by all auth users. Insert by active users **except Auditors**. Update/Delete disabled. |
...
| `bulk_action_snapshots` | Pre-action snapshots of affected rows for every bulk approve/reject, enabling one-step undo. | Insert/Update by active users **except Auditors**. |
...
# Optional — powers the in-app Desmos Graphing Calculator modal on Math
# questions. Free key from https://www.desmos.com/api. The app still runs
# without it; the calculator modal just shows a setup message instead.
...
3. Click **New Query** and copy-paste the entire contents of [`supabase/schema.sql`](supabase/schema.sql).
...
## 🩺 Recent Maintenance Pass (Performance, Security & Code Quality)

A diagnostic pass was done covering performance, security, and code hygiene. **The reviewer workflow, question data, and review states behave exactly as before** — this was not a feature change. Specifically:

* **No change** to how questions are approved/rejected/claimed, checklist logic, consensus sampling/resolution, or any already-reviewed question's stored status, checks, comments, or consensus history.
* **Performance**: the filter/sort/stats pipeline and per-question audit-log lookups are now memoized instead of being recomputed from scratch on every click; bulk saves are sent in smaller batches instead of all at once. This fixed a freeze that occurred when interacting with questions after a workspace/log history had grown large — it affected all question types, not just one section.
* **Two genuine bug fixes** (behavior restored to what was intended, not new behavior):
  * Editing the "Supporting Graphic URL" field on a question now actually persists — it was previously silently discarded on save.
  * A question loaded with a missing `difficulty` value no longer crashes its card (falls back to displaying "medium" rather than throwing).
* **Security hardening**: the read-only Auditor role is now enforced at the database (RLS) level for `questions`, `audit_log`, and `bulk_action_snapshots`, in addition to the existing UI restrictions. Re-run the updated `supabase/schema.sql` against your Supabase project for this to take effect — it is not applied automatically by a frontend deploy.
* **Code cleanup**: removed unused imports/variables and duplicated logic across several components; no visual or behavioral change.

### Known issue found during this pass (not changed — flagging for a decision)
The per-question **rejection webhook** (`rejection_webhook_url`, referenced in the workflow diagram above as "Export Option: Webhooks") is implemented but not currently reachable from the UI — no button in the app triggers it today, including the bulk reject actions. Admins can still configure the webhook URL in Settings, but nothing will call it until this is wired up. Left untouched pending a decision on whether to connect it or remove it.