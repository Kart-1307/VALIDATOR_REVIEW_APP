...
export default function DomainAnalytics({ questions, onSelectSubdomainFilter }: DomainAnalyticsProps) {
  ...
  const analyticsData = useMemo(() => {
    ...
    questions.forEach(q => {
      ...
      // A question can be claimed (locked by a validator for review) while its
      // overall status is still pending — track that overlap separately so it
      // doesn't get lost inside the general "pending" bucket.
      const isClaimedPending = status === 'pending' && !!q.claimedBy;
  ...
  // Reset the sub-domain choice whenever the domain choice changes, so a
  // stale sub-domain name from a different domain can't linger selected.
  React.useEffect(() => {
    setSelectedSubdomain('all');
  }, [selectedDomain]);

  // Domain dropdown options, alphabetized for easy scanning.
  const domainOptions = useMemo(
    () => [...analyticsData].map(d => d.name).sort((a, b) => a.localeCompare(b)),
    [analyticsData]
  );

  // The full record for whichever domain is picked in the dropdown (if any).
  const selectedDomainData = useMemo(
    () => (selectedDomain === 'all' ? null : analyticsData.find(d => d.name === selectedDomain) || null),
    [analyticsData, selectedDomain]
  );

  // Sub-domain dropdown options, scoped to the chosen domain.
  const subdomainOptions = useMemo(
    () => (selectedDomainData ? Object.keys(selectedDomainData.subdomains).sort((a, b) => a.localeCompare(b)) : []),
    [selectedDomainData]
  );

  // The single sub-domain record, when both dropdowns are set to specific values.
  const selectedSubdomainData = useMemo(() => {
    if (!selectedDomainData || selectedSubdomain === 'all') return null;
    return selectedDomainData.subdomains[selectedSubdomain] || null;
  }, [selectedDomainData, selectedSubdomain]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. Control Row: Domain/Sub-domain pickers, grouping selector & search */}
      <div className="bg-[#fafafa] border border-[#e4e4e7] rounded-xl p-4 space-y-4">

        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <label className="flex items-center gap-1.5 text-xs font-bold text-zinc-500 mb-1.5 select-none">
              <BookOpen className="w-3.5 h-3.5 text-indigo-600" /> Domain
            </label>
            <select
              value={selectedDomain}
              onChange={(e) => setSelectedDomain(e.target.value)}
              className="w-full px-3 py-1.5 border border-[#e4e4e7] rounded-lg text-xs font-semibold bg-white text-zinc-900 focus:outline-none focus:ring-1 focus:ring-[#6366f1] focus:border-[#6366f1] cursor-pointer"
            >
          ...
          <div className="flex-1">
            <label className="flex items-center gap-1.5 text-xs font-bold text-zinc-500 mb-1.5 select-none">
              <Layers className="w-3.5 h-3.5 text-indigo-600" /> Sub-domain
            </label>
            <select
              value={selectedSubdomain}
              onChange={(e) => setSelectedSubdomain(e.target.value)}
              disabled={selectedDomain === 'all'}
              className="w-full px-3 py-1.5 border border-[#e4e4e7] rounded-lg text-xs font-semibold bg-white text-zinc-900 focus:outline-none focus:ring-1 focus:ring-[#6366f1] focus:border-[#6366f1] cursor-pointer disabled:bg-[#f2f2f3] disabled:text-zinc-400 disabled:cursor-not-allowed"
            >
              <option value="all">{selectedDomain === 'all' ? 'Choose a domain first' : 'All Sub-domains'}</option>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center">
          ...
          {/* Local Search — only meaningful while browsing all domains */}
          {selectedDomain === 'all' && (
      ...
      {/* 2b. Focused snapshot when a specific domain + sub-domain are both picked */}
      {selectedDomainData && selectedSubdomainData && (
        <div className="bg-[#fafafa] border border-[#e4e4e7] rounded-xl p-5">
          <div className="flex items-center justify-between gap-2 mb-4">
            <div>
              <p className="text-[11px] uppercase font-bold text-zinc-500 tracking-wider">
                {selectedDomainData.name} • {getGroupingLabel(selectedGrouping)}
              </p>
              <h4 className="text-sm font-bold text-zinc-900 mt-0.5" title={selectedSubdomainData.name}>
                {selectedSubdomainData.name}
              </h4>
            </div>
            <button
              onClick={() => onSelectSubdomainFilter(selectedDomainData.name, selectedSubdomainData.name, selectedGrouping)}
              className="flex items-center gap-1.5 text-xs font-bold text-white bg-[#6366f1] hover:bg-indigo-600 px-3 py-1.5 rounded-lg transition-all cursor-pointer shrink-0"
            >
              Go to curate <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="bg-white border border-[#e4e4e7] rounded-lg p-3 text-center">
              ...
              <p className="text-lg font-extrabold text-zinc-900 font-mono">{selectedSubdomainData.pending}</p>
            </div>
            <div className="bg-white border border-amber-200 rounded-lg p-3 text-center">
              <p className="text-[10px] uppercase font-bold text-amber-700 tracking-wider">Claimed (Pending)</p>
              <p className="text-lg font-extrabold text-amber-700 font-mono">{selectedSubdomainData.claimedPending}</p>
            </div>
            <div className="bg-white border border-rose-200 rounded-lg p-3 text-center">
              <p className="text-[10px] uppercase font-bold text-rose-600 tracking-wider">Rejected</p>
              <p className="text-lg font-extrabold text-rose-600 font-mono">{selectedSubdomainData.rejected}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 gap-5 ${selectedDomain === 'all' ? 'lg:grid-cols-2' : ''}`}>
        <AnimatePresence mode="popLayout">
          {(selectedDomain === 'all' ? filteredAnalytics : (selectedDomainData ? [selectedDomainData] : [])).map((domain, idx) => {
                ...
                <div>
                  ...
                  <div className="space-y-1.5 mb-5">
                    ...
                    {domain.claimedPending > 0 && (
                      <div className="text-[11px] font-mono font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-2 py-1">
                        {domain.claimedPending} of those pending {domain.claimedPending === 1 ? 'item is' : 'items are'} claimed, awaiting review
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 border border-[#e4e4e7] rounded-lg p-2 max-h-[220px] overflow-y-auto bg-white">
                    {(Object.values(domain.subdomains) as Array<{ name: string; total: number; approved: number; rejected: number; pending: number; claimedPending: number; }>).map((sub) => {
                      ...
                      return (
                        <div 
                          key={sub.name}
                          className={`flex items-center justify-between text-xs py-2 px-2.5 rounded-md border transition-all group ${
                            isHighlighted
                              ? 'bg-indigo-50 border-[#6366f1] ring-1 ring-[#6366f1]'
                          ...
                          <div className="space-y-0.5 truncate pr-2">
                            ...
                            <div className="text-[11px] text-zinc-500 font-mono flex items-center gap-2 flex-wrap">
                              ...
                              {sub.claimedPending > 0 && (
                                <>
                                  <span>•</span>
                                  <span className="text-amber-700" title="Claimed by a validator but still pending review">
                                    {sub.claimedPending} claimed (pending)
                                  </span>
                                </>
                              )}
                            </div>
          ...
          {selectedDomain === 'all' && filteredAnalytics.length === 0 && (
          ...
          {selectedDomain !== 'all' && !selectedDomainData && (
            <div className="py-12 text-center text-zinc-500 text-xs italic">
              No data found for the selected domain.
            </div>
          )}
        </AnimatePresence>
      </div>
