  ...
  return (
    <div className={`flex-1 min-w-0 rounded-xl border ${accentBorder} bg-white flex flex-col overflow-hidden`}>
      ...
      <div className="p-4 space-y-3 overflow-y-auto max-h-[55vh]">
        {question.passage && (
          <div>
            <TableAwareText text={question.passage} textClassName="text-xs text-zinc-800 leading-relaxed font-medium" />
          </div>
        )}

        {question.stimulus && (
          <div>
            <TableAwareText text={question.stimulus} textClassName="text-xs text-zinc-800 leading-relaxed font-medium" />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-2">
          {/* Bug fix: choices can be null/undefined for grid-in questions —
              Object.entries(null) throws and crashes the whole app since
              there's no error boundary. Guard with `|| {}`. */}
