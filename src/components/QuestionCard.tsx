...
import { isMathQuestion, extractExpressions, buildDesmosSolution, analyzeDistractors, suggestDistractors, DesmosSolution } from '../lib/mathTools';
import { fetchAiDesmosSolution } from '../lib/aiDesmosSolution';
import { TableAwareText } from '../lib/tableText';

// Hoisted out of the component: these are pure and don't depend on
// props/state, so they don't need to be re-declared on every render of
// every card.
...
function getSectionLabel(sect?: string, section?: string) {
...
interface QuestionCardProps {
  ...
  onNext?: () => void;
  hasNext?: boolean;
  // Admin-only: open the per-question revert history drawer for this item.
  onOpenHistory?: (question: SATQuestion) => void;
}
...
function QuestionCard({
  ...
  // AI-generated, question-specific solution (falls back to the static
  // template in mathTools.ts if this isn't configured or fails). Cached by
  // question id so re-toggling the panel doesn't re-fetch every time, and
  // cleared out if the question underneath changes (e.g. paging to a
  // different item while the panel state persists).
  const [aiSolution, setAiSolution] = useState<{ questionId: string; solution: DesmosSolution } | null>(null);
  const [aiSolutionStatus, setAiSolutionStatus] = useState<'idle' | 'loading' | 'error'>('idle');
  ...
  // Fetch the AI-generated, question-specific solution the first time the
  // panel is opened for a given question. Doesn't refetch just because the
  // panel is toggled closed/open again, and doesn't refetch on every
  // keystroke elsewhere in the card since it's keyed only on questionId +
  // whether the panel is open.
  useEffect(() => {
    if (!isMath || !isDesmosSolutionOpen) return;
    if (aiSolution?.questionId === question.id) return; // already have it (or already tried)
    if (aiSolutionStatus === 'loading') return;

    let cancelled = false;
    setAiSolutionStatus('loading');
    fetchAiDesmosSolution(question)
      .then((solution) => {
        if (cancelled) return;
        setAiSolution({ questionId: question.id, solution });
        setAiSolutionStatus('idle');
      })
      .catch(() => {
        // Silent fallback to the static template — the render below already
        // handles aiSolutionStatus === 'error' by using buildDesmosSolution.
        if (cancelled) return;
        setAiSolutionStatus('error');
      });
  ...
  }, [isMath, isDesmosSolutionOpen, question.id]);

  // Bug fix: this used to be a plain `const`, so extractExpressions's regex
  // pass over stimulus+question+passage text reran on every render of every
  // Math-tagged card, even when nothing about the question had changed and
  // even when neither the Desmos calculator nor the solution guide was open.
  // Memoizing it means the (potentially large) text is only re-scanned when
  // the question's own text actually changes.
  const mathExpressions = useMemo(
    () => (isMath ? extractExpressions(question) : []),
    [isMath, question.stimulus, question.question, question.passage]
  );
  ...
  const allCategories = useMemo(
    () => Array.from(new Set([...DEFAULT_CATEGORIES, ...(availableCategories || [])])).sort(),
    [availableCategories]
  );
  ...
  return (
    ...
    >
      ...
      {question.passage && (
        <div className="my-1">
          <TableAwareText text={question.passage} textClassName="text-sm text-zinc-800 leading-relaxed tracking-wide font-medium" />
        </div>
      )}
      ...
      {question.stimulus && (
        <div className="my-1">
          <TableAwareText text={question.stimulus} textClassName="text-sm text-zinc-800 leading-relaxed font-medium" />
        </div>
      )}
      ...
      {/* Main Question Text */}
      <div>
        <TableAwareText text={question.question} textClassName="text-sm font-medium text-zinc-800 leading-relaxed" />
      </div>

      {/* Multiple Choices (A, B, C, D) */}
      {/* Bug fix: `question.choices` can be null/undefined for grid-in
          questions (there are no A/B/C/D options to store), and
          Object.entries(null) throws a TypeError with no error boundary
          to catch it — this was crashing/freezing the whole app the
          instant a grid-in question rendered. Guard with `|| {}` and skip
          the block entirely when there's nothing to show. */}
      ...
      {/* Desmos-style step-by-step solution (Enhancement §2). Prefers the
          AI-generated, question-specific write-up; falls back to the static
          keyword-matched template in mathTools.ts if the Edge Function isn't
          configured or the call fails, so this never leaves the panel empty. */}
      {isMath && isDesmosSolutionOpen && (() => {
        const usingAi = aiSolution?.questionId === question.id;
        const isLoading = aiSolutionStatus === 'loading' && !usingAi;
        const solution = usingAi ? aiSolution!.solution : buildDesmosSolution(question);
        return (
          <div className="border border-sky-200 rounded-xl overflow-hidden bg-sky-50">
            <div className="px-4 py-2.5 bg-sky-50 border-b border-sky-200 text-xs font-bold text-sky-300 uppercase tracking-wide flex items-center justify-between gap-1.5">
              <span className="flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" /> Digital SAT-Style Solution, with Desmos
              </span>
              {!usingAi && !isLoading && (
                <span className="text-[10px] normal-case font-semibold text-zinc-500" title="Set ANTHROPIC_API_KEY on the generate-desmos-solution Edge Function to enable a solution tailored to this exact question.">
                  Generic template (AI solution unavailable)
                </span>
              )}
            </div>
            {isLoading ? (
              <div className="p-4 text-xs text-zinc-500 italic">Generating a solution for this specific question…</div>
            ) : (
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard?.writeText(s.value)
                            ...
                            .catch(() => {
                              // Clipboard access denied/unavailable — don't claim success.
                            });
                        }}
      ...
      <div className="border border-[#e4e4e7] rounded-xl overflow-hidden bg-[#f2f2f3]">
        <div className="w-full flex justify-between items-center px-4 py-2.5 bg-[#fafafa] hover:bg-[#f2f2f3]/80 transition-all text-xs font-bold text-zinc-500 select-none">
          <button
            type="button"
            onClick={() => setIsAuditOpen(!isAuditOpen)}
            className="flex items-center gap-1.5 uppercase tracking-wide cursor-pointer"
          >
          ...
          <div className="flex items-center gap-2">
            {isAdmin && onOpenHistory && (
              <button
                ...
                title="View this question's revert history and restore a past state (admin only)"
                className="flex items-center gap-1 px-2 py-1 text-[11px] font-bold text-indigo-600 bg-white border border-indigo-200 rounded-full hover:bg-indigo-600 hover:text-white transition-all cursor-pointer"
              >
                <RotateCcw className="w-3 h-3" /> Revert History
              </button>
            )}
...
// NOTE: memoizing here helps once a card's props are actually stable (e.g.
// the auditLogs array reference from App.tsx's new logsByQuestionId map, or
// availableCategories from the memoized uniqueCategories list). The handler
// props (onApprove, onReject, etc.) are still re-created on every App render
// today, which limits how much this alone can skip — fully stabilizing those
// with useCallback is a larger follow-up refactor, not required to fix the
// freeze reported here (that was dominated by the full-dataset recompute and
// per-card log scan, both fixed in App.tsx).
