...
// Security fix: this was previously a wildcard ('*'), which is broader than
// necessary for an admin-only, credential-checked endpoint. Falls back to
// '*' only when SITE_URL isn't configured (e.g. local dev), so this doesn't
// break existing deployments that haven't set it yet — but production
// deployments should set SITE_URL to lock this down.
