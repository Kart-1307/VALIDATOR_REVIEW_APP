...
-- Security fix: this used to check only `active = true`, so the "read-only
-- auditor" role was enforced purely in the client (blockIfAuditor() /
-- disabled buttons). An authenticated auditor could bypass all of that by
-- calling the Supabase REST API directly with their own JWT and update any
-- question. Auditors are explicitly excluded here so read-only is a real,
...
-- Security fix: same gap as the questions UPDATE policy above — auditors
-- were "active" and so could insert audit_log rows directly via the API,
-- even though the UI never lets them trigger a logged action. Excluded here
-- too, since audit_log is meant to be an immutable, trustworthy record.
...
-- ---------------------------------------------------------------------------
-- 6. BULK ACTION SNAPSHOTS — undo support for bulk approve/reject.
-- Stores an exact pre-action copy of every question a bulk action is about
-- to touch, so "Undo" can restore precisely those questions to precisely
-- what they looked like before — never a blanket reset that could also
-- clobber unrelated, legitimately-approved/claimed questions (see incident
-- where a manual recovery reset accidentally touched pre-existing approvals).
-- ---------------------------------------------------------------------------
create table if not exists public.bulk_action_snapshots (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  ...
  -- Full pre-action copy (app-shape JSON) of every affected question.
  snapshot jsonb not null default '[]'::jsonb,
  undone boolean not null default false,
  undone_at timestamptz
);

alter table public.bulk_action_snapshots enable row level security;

drop policy if exists "Snapshots are viewable by authenticated users" on public.bulk_action_snapshots;
create policy "Snapshots are viewable by authenticated users"
  on public.bulk_action_snapshots for select
  to authenticated
  using (true);

-- Security fix: same auditor gap as questions/audit_log above.
drop policy if exists "Active users can create snapshots" on public.bulk_action_snapshots;
create policy "Active users can create snapshots"
  on public.bulk_action_snapshots for insert
  to authenticated
  with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.active = true and p.role != 'auditor'));

drop policy if exists "Active users can mark snapshots as undone" on public.bulk_action_snapshots;
create policy "Active users can mark snapshots as undone"
  on public.bulk_action_snapshots for update
  to authenticated
  using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.active = true and p.role != 'auditor'))
  with check (true);
...
begin
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'bulk_action_snapshots'
  ) then
    alter publication supabase_realtime add table public.bulk_action_snapshots;
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- 7. First admin — run this AFTER you've signed up once in the app, so your
